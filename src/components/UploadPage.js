   ...
   /*      if(){
    ...
    return (
        <div id="upload-container">
            ...
            <Divider/>
            {/* 판매자명 */}
            <Form.Item label={<span className='upload-label'>판매자명</span>} name="seller" rules={[{required: true,
                message: '판매자명을 입력해주세요'}]}>
                <Input className='upload-name' placeholder='이름을 입력해주세요' size='large' />
            </Form.Item>
            <Divider/>
            {/* 상품명 */}
            <Form.Item label={<span className='upload-label'>상품명</span>} name="name" rules={[{required: true,
                message: '상품명은 필수 입력사항입니다.'}]}>
                <Input className='upload-name' placeholder='상품명을 입력해주세요' size='large' />
            </Form.Item>
            <Divider/>
            {/* 판매가 */}
            <Form.Item label={<span className='upload-label'>판매가</span>} name="price" rules={[{required: true,
                message: '판매가를 입력해주세요.'}]}>
                <InputNumber className='upload-price' min={0} size='large' />
            </Form.Item>
            <Divider/>
            {/* 상품설명 */}
            <Form.Item label={<span className='upload-label'>상품명</span>} name="description" rules={[{required: true,
                message: '상품설명을 입력해주세요.'}]}>
                <Input.TextArea id="prodcut-description" showCount maxLength={300} placeholder='상품설명을 입력해주세요' size='large' />
            </Form.Item>
            <Divider/>
            {/* 버튼 */}
            <Form.Item>
